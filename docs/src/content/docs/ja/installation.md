...
ドキュメントを再開し、ホーム画面は最近見た・お気に入り・よく参照する
ドキュメントをまとめ、整理が必要な箇所への導線も示すダッシュボードです。UI 全体を
Gmail / GitHub 流のキーボード操作で扱えます。Ctrl+P（または `/`）で
即時検索、Ctrl+N（または `c`）で新規ドキュメント、`g` で始まる
シーケンスで画面移動、`j` / `k` でリストを移動でき、`?` で
ショートカット一覧を表示します。統計画面では件数・embedding
カバレッジ・整理のヒントを確認できます。★を付けたドキュメントはサイドバー上部に

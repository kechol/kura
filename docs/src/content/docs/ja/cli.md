...
- **ドキュメントの指定** — ドキュメントを取るコマンドは、doc key・
  `#key`・タイトルを受け付けます（例: `kura get "今日のメモ"` や
  `kura get #a1b2c3`）。
...
| コマンド | 内容 |
|---|---|
| `kura add <file>` | 追加。`-` で標準入力。`--tags` / `--title` / `--bucket` |
| `kura get <ref>` | 表示（Markdown または `--json`） |
| `kura edit <ref>` | `$EDITOR` で編集。frontmatter でタイトルとタグも変更 |
| `kura rm <ref>` | 削除 |
| `kura mv <ref> <title>` | リネーム（相互リンクを更新） |
| `kura ls` | 一覧。`--tag` / `--bucket` / `--sort` / `--stale` |
| `kura clip <url>` | Web ページを LLM で整形して取り込み |
| `kura export --dir <d>` | frontmatter 付き Markdown で書き出し |
| `kura import <dir>` | `kura_key` でラウンドトリップ（更新 or 新規） |

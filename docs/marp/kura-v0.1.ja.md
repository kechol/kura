...
- **フォルダ階層は持たない** — Bucket・階層タグ・相互リンクで整理（Cosense 方式）
...
場所にまたがった瞬間、その決断は破綻します。だから kura は、階層その
ものを捨てました。
...
LLM プロバイダが見つからなくても、kura は止まりません。
...
この 4 つは、**モデルなしでいつでも動きます**。セマンティック検索・
リランク・clip 整形・タグ提案・クエリ展開は、プロバイダが無ければ
そっと警告を出して代替に切り替わります。

飛行機の中でも困りません。手放すのは、本当にモデルが要る機能だけ。
Ollama も LM Studio も、あれば勝手に見つけます。

...
- **自己組織化**: ファイリングの強制なし。まず Bucket のルート（inbox）に放り込み、あとで `kura triage` でバックログを整理できる。Bucket + 階層タグ（`tech/db/sqlite`）+ 相互リンク（`[[タイトル]]`）で整理し、フォルダ風の名前が欲しいときだけ任意のドキュメント path（`db/sqlite`）を付けられる
...
# バックログを整理（まず放り込み、あとで整理）
kura triage                    # 未整理・タグなしのドキュメントごとに、マージ・タイトル・タグ・path・リンクを提案
kura audit                     # 健全性レポート: 未解決リンク・重複タグ・重複ドキュメント・矛盾
kura audit links               # 未解決リンクの一覧のみ（旧 kura link broken）

# メンテナンス
kura status                    # 統計（embedding カバレッジ・陳腐化候補・整理待ちなど）
...
起動すると最後に読んでいたドキュメントに戻ります。ホームは、最近見た・更新した・作成した・お気に入り・よく参照するドキュメントに加え、整理が必要な箇所への導線をまとめたダッシュボードです。統計画面では、整理が必要な箇所（未整理・タグなし・孤立ドキュメント、未解決リンク、表記ゆれタグ）を数え、それぞれ対処に使う CLI コマンドを示します。

UI 全体を Gmail / GitHub 流のキーボード操作で扱えます。`Ctrl + P`（または `/`）で検索モーダルが開き、入力しながら結果が絞り込まれます。`Ctrl + N`（または `c`）で新しいドキュメントを作成し、`g` で始まるシーケンスで画面を移動（`g d` で一覧、`g g` でグラフ）、`j` / `k` でリストを移動して `Enter` で開き、ドキュメント画面では `s` でお気に入り、`e` で編集開始、`Escape` でエディタから抜けられます。`?` でショートカット一覧を表示します。

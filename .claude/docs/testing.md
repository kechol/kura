...
| `tests/config.test.ts` | unit | Defaults match SPEC §11, TOML serialize/parse round-trip, merge-onto-defaults, unknown/type/invalid-range fallback, inherited-property rejection, leaf-only dotted set plus enum/finite/integer/range validation, flat listing. |
| `tests/documents.test.ts` | unit | Repository invariants: derived-table sync, atomic public tag mutations and FTS rollback, link auto-resolution (case-insensitive, bucket-scoped), duplicate full-path rules, chunk rebuild only on content change, rename relinking, delete cleanup, `resolveDoc` ambiguity, `touchAccess`, literal hierarchy/list filters, import/export round-trip (incl. the numeric doc_key regression and the favorite flag: an absent frontmatter key leaves the pin alone, an explicit `false` clears it), `setFavorite` leaving `updated_at` alone, tag/link/bucket core functions. |
| `tests/aliases.test.ts` | unit + e2e | Aliases: FTS `aliases`-column sync + keyword match, title/case-dup skip and invalid rejection, `updateDocument` add-only vs `setAliasesForDoc` replace, cascade on delete; `[[alias]]` resolution (stage 3, self-heal on later add / re-unresolve on remove, title beats alias, shared alias stays unresolved), `resolveDoc` unique/ambiguous alias, frontmatter round-trip, `kura alias add/ls/rm` CLI. |
| `tests/revisions.test.ts` | unit + e2e | Revision snapshots via `updateDocument` (content/title/path change, burst coalescing, metadata-only no-op, restore-snapshots-first); `snapshotRevision` dedup + `MAX_REVISIONS_PER_DOC` pruning; `stateAsOf` time coverage; `kura history` list/show/restore and `kura get --as-of` CLI. |
| `tests/changes.test.ts` | unit + e2e | `parseSince` relative/absolute/garbage, `changesSince` created-vs-updated split + rename detection against revisions, bucket filter + limit, `kura changes --since` CLI with `--json`. |
| `tests/paths.test.ts` | unit | Document paths: `normalizeDocPath` / `joinDocPath` / `replaceWikiLinkTargets`, per-path uniqueness (incl. cross-form full-path collisions), two-stage link resolution (ambiguity, stickiness, doctor bulk pass), `resolveDoc` full-path/ambiguity messages, rename/move rewrite matrix, literal `%` / `_` / backslash prefix matching, `moveDocumentsByPrefix` rollback + guards, `createDocumentWithRetry` suffixes, import path fallback, `buildDocTree` / `docTree` (branch/leaf structure, merge, literal-`/` titles). |
...
| `tests/filing.test.ts` | unit (mock provider) | Path-suggestion engine (`src/core/filing.ts`, now the `kura triage` path step): structural link/tag voting with evidence, LLM pick (mocked, normalized, `isNew`), unusable-answer fallback, no-signal and no-provider degraded paths. |
| `tests/triage.test.ts` | unit (mock provider) + e2e | `listTriageBacklog` predicates (unfiled OR untagged, `triaged_at` exclusion, `redo`, `updated_at DESC` + limit), `markTriaged` leaving `updated_at` alone, `triageDocument` per-step (title/tags/path-unfiled-only, provider-null degraded report that aggregates warnings and skips LLM steps); `kura triage` CLI `--json` shape mutating nothing, `--apply` filing + stamping + `--redo`, `--json`/`--apply` exclusivity (exit 2), bad `--steps` (exit 2), status backlog line. |
| `tests/dedupe.test.ts` | unit (mock provider) | `exactDuplicates` (byte-identical), `nearDuplicates` (vector candidate + LLM verdict, no-provider warning), `judgeDuplicatePair` symmetric cache, `mergeDuplicate` (survivor gains the duplicate's title alias + tags), `appendRelatedLinks` (`## 関連` idempotent, byte-preserving), `suggestTitleForDocument` / `suggestLinksForDocument` degraded paths. |
...
| `tests/search.test.ts` | unit (mock provider) | Trigram keyword search (BM25 title weighting, `--all` AND, literal hierarchy filters, <3-char LIKE fallback, query escaping); immutable embedding identity, bounded/resumable backfill, edit/delete/config races across provider awaits, atomic reset, KNN prefilter + adaptive unique-document expansion (including literal tag paths); hybrid fusion/rerank/expand, provider-setting cache invalidation, worker settlement on failure, degraded mode, `parseYesNo`, `blendScores`. |
...
| `tests/m6.test.ts` | unit (mock provider) | Clip extract/format (readability, turndown fallback, LLM formatting + `clip` cache, tag suggestion + `tag` cache, script stripping), gardening (levenshtein, merge candidates, oversized tags), stale scoring, all `doctor` fixes incl. `chunks_vec` recreation and `retokenizeFts`. |
...
| `tests/commands-io.test.ts` | e2e | `import`/`export` round-trip across two homes (kura_key stability), hostile `.` / `..` / special paths, sanitized and key-suffix collisions, directory/leaf symlinks and hard-link outside sentinels, `--tag` filtered export, path-to-directory export + frontmatter/dir-derived path import, invalid-frontmatter skip + all-skipped exit 1, `bucket` add/ls/mv/rm semantics. |
...
| `tests/api.test.ts` | integration | REST endpoints (SPEC §8.2) against `startServer` on port 0: stats, buckets, docs CRUD + composite-filter pagination, malformed/wrong-type body 400s with no mutation, strict positive-integer boundaries, whole-state rollback for failed `PUT`, path conflicts, favorite semantics, related, three search modes + errors, tags, projected graph semantics/scope/large-body omission and maximum-safe-integer stale-day handling, SPA fallback, 404 JSON. |
...
| `tests/editor-save.test.tsx` | component | Mounted inline editor and DocDetail lifecycle: debounce-unmount flush, serialized delayed saves, failure/retry, latest-body retention after failed metadata refresh, body/metadata retry separation, same-doc metadata refresh, A→B isolation, and Japanese IME completion. |

...
| `tests/filing.test.ts` | unit (mock provider) | `kura mv suggest` core: unfiled listing, structural link/tag voting with evidence, LLM pick (mocked, normalized, `isNew`), unusable-answer fallback, no-signal and no-provider degraded paths. |

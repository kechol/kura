...
| `llm.provider` | `"auto" \| "ollama" \| "lmstudio" \| "none"` | `"auto"` | `auto` probes Ollama then LM Studio (`src/core/llm/provider.ts`, result cached in-process for 60 s). A named provider is used only if reachable. `none` disables all LLM features: `vsearch`/`embed`/`audit contradictions` exit 4, while `query`/`clip`/`triage`/`audit` (`tags`/`dupes`) degrade with warnings. |
...
| `llm.models.generation` | string | `"qwen3:4b"` | Used for clip formatting, query expansion, answer generation, the `kura triage` engines (tag/title suggestion, path/duplicate/link judgment), and the contradiction audit (`src/core/clip/format.ts`, `src/core/search/expand.ts`, `src/core/search/ask.ts`, `src/core/{tagging,titling,filing,dedupe,linking,audit}.ts`). Cache keys include the model. |
